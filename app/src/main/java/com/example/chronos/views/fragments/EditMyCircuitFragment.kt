...
class EditMyCircuitFragment : Fragment(R.layout.fragment_edit_my_circuit) {
    companion object {
        const val TAG: String = "EditMyCircuitFragment"
    }

    private var _binding: FragmentEditMyCircuitBinding? = null
    private val binding get() = _binding!!
    private val viewModel: EditMyCircuitViewModel by viewModels {
        ViewModelProvider.provideEditMyCircuitViewModelFactory(requireContext().applicationContext)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        _binding = FragmentEditMyCircuitBinding.bind(view)
        observeViewModel()
    ...
    private fun observeViewModel()
    {
        viewModel.circuit.observe(viewLifecycleOwner) {
            binding.textFieldTitle.editText?.setText(it.title)
